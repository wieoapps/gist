...
// db is how every function in this package reaches the raw MySQLServiceClient -
// gistsdk.Server never exposes it directly (see rpcconn's package doc).
func (t *Transaction) db() gistproto.MySQLServiceClient {
	return rpcconn.MustFor(t.server).DB
}
...
func (t *Transaction) Commit() error {
	...
	ack, err := t.db().Commit(context.Background(), &gistproto.TransactionHandle{TransactionHandle: t.handle})
	if err != nil {
		return fmt.Errorf("gistmysql: commit: %w", err)
	}
	if ack.GetErrorCode() != "" {
		return fmt.Errorf("gistmysql: commit: %s: %s", ack.GetErrorCode(), ack.GetErrorMessage())
	}
	return nil
}
...
func (t *Transaction) Rollback() error {
	...
	ack, err := t.db().Rollback(context.Background(), &gistproto.TransactionHandle{TransactionHandle: t.handle})
	if err != nil {
		return fmt.Errorf("gistmysql: rollback: %w", err)
	}
	if ack.GetErrorCode() != "" {
		return fmt.Errorf("gistmysql: rollback: %s: %s", ack.GetErrorCode(), ack.GetErrorMessage())
	}
	return nil
}
