...
type widgetModel struct {
	ID   int    `source:"db.widgets" db:"id" json:"id"`
	Name string `db:"name" json:"name"`
}

// newFakeTransaction builds a *Transaction bound to a fresh fakeDBClient,
...
func newFakeTransaction(t *testing.T, fake *fakeDBClient) *Transaction {
	t.Helper()
	server := &gist.Server{}
	rpcconn.Register(server, &rpcconn.Clients{DB: fake})
...
func TestFind_FirstOp_NamedTransactionForwardsName(t *testing.T) {
	fake := &fakeDBClient{}
	server := &gist.Server{}
	rpcconn.Register(server, &rpcconn.Clients{DB: fake})
	svc := NewService(server, "svc")
	tr, err := svc.NewNamedTransaction(context.Background(), "my-tx")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if _, err := Find[widgetModel](tr); err != nil {
...
func TestFind_TransportError_Propagates(t *testing.T) {
	fake := &fakeDBClient{repoErr: context.DeadlineExceeded}
	tr := newFakeTransaction(t, fake)
	if _, err := Find[widgetModel](tr); err == nil {
...
func TestSaveWithReturning_DecodesReturnedRows(t *testing.T) {
	...
	// tag) is deliberately independent of the return type (widgetModel)
	// - the exact decoupling fix applied to gist-mysql this session
	// (mirroring the same fix gist-postgres needed from the start).
	...
	// its full "db.widgets" source), not the insert payload type
...
func TestUpdate_SendsPartialRowAndConditions(t *testing.T) {
	...
	type nameUpdate struct {
		Name string `source:"db.widgets" db:"name" json:"name"`
	}
...
func TestUpdate_NilPointerFieldOmittedFromPayload(t *testing.T) {
	// The exact partial-update semantic that was a real bug source this
	...
	_, err := Update(tr, partialUpdate{Name: nil, Qty: 0}, WithConditions(NewCondition("id", Equal, 1)))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	var row map[string]any
	_ = json.Unmarshal(fake.repoReq.GetRows()[0].GetRowJson(), &row)
	if _, present := row["name"]; present {
		t.Errorf("expected a nil *string field to be omitted from the payload entirely, got %v", row)
	}
	if v, present := row["qty"]; !present || v != float64(0) {
		t.Errorf("expected a non-pointer zero value to still be included, got %v", row)
	}
}
...
func TestFind_WithSorting_Forwarded(t *testing.T) {
	fake := &fakeDBClient{}
	tr := newFakeTransaction(t, fake)

	_, err := Find[widgetModel](tr, WithSorting(Desc{"name"}))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	sorts := fake.repoReq.GetOptions().GetSorts()
	if len(sorts) != 1 || !sorts[0].GetDescending() {
		t.Fatalf("expected a descending sort forwarded, got %v", sorts)
	}
}
