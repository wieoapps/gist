...
func TestBuildModelSchema_ScalarFields(t *testing.T) {
	...
	if len(schema.Sources) != 2 || schema.Sources[0] != "db" || schema.Sources[1] != "orders" {
...
func TestBuildModelSchema_BoolField_SetsIsBool(t *testing.T) {
	// The wire hint that lets the server coerce a driver-returned
	// TINYINT/BIT number into a real JSON boolean before sending the row
	// back - see gist-server/services/gist-mysql/scanner.go's
	// coerceBoolFields. Covers both a plain bool and a *bool (the common
	// nullable-column shape) reflecting to the same IsBool: true.
	type flags struct {
		ID         int   `source:"db.flags" db:"id" json:"id"`
