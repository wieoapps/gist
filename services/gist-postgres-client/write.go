...
func Save(tr *Transaction, models ...any) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("gistpostgres: save: %w", err)
	}
	tr.captureHandle(resp)
	if resp.GetErrorCode() != "" {
		return 0, fmt.Errorf("gistpostgres: save: %s: %s", resp.GetErrorCode(), resp.GetErrorMessage())
	}
...
func SaveWithReturning[model any](tr *Transaction, models ...any) ([]model, int64, error) {
	...
	resp, err := tr.pg().Repo(context.Background(), req)
	if err != nil {
		return nil, 0, fmt.Errorf("gistpostgres: save with returning: %w", err)
	}
	tr.captureHandle(resp)
	if resp.GetErrorCode() != "" {
		return nil, 0, fmt.Errorf("gistpostgres: save with returning: %s: %s", resp.GetErrorCode(), resp.GetErrorMessage())
	}
...
func Update(tr *Transaction, newValues any, opts ...Option) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("gistpostgres: update: could not encode row: %w", err)
	}
	...
	resp, err := tr.pg().Repo(context.Background(), req)
	if err != nil {
		return 0, fmt.Errorf("gistpostgres: update: %w", err)
	}
	tr.captureHandle(resp)
	if resp.GetErrorCode() != "" {
		return 0, fmt.Errorf("gistpostgres: update: %s: %s", resp.GetErrorCode(), resp.GetErrorMessage())
	}
	return resp.GetAffected(), nil
}
...
func UpdateWithReturning[model any](tr *Transaction, newValues any, opts ...Option) ([]model, int64, error) {
	...
	rowJSON, err := json.Marshal(buildModelRow(reflect.ValueOf(newValues)))
	if err != nil {
		return nil, 0, fmt.Errorf("gistpostgres: update with returning: could not encode row: %w", err)
	}
	...
	resp, err := tr.pg().Repo(context.Background(), req)
	if err != nil {
		return nil, 0, fmt.Errorf("gistpostgres: update with returning: %w", err)
	}
	tr.captureHandle(resp)
	if resp.GetErrorCode() != "" {
		return nil, 0, fmt.Errorf("gistpostgres: update with returning: %s: %s", resp.GetErrorCode(), resp.GetErrorMessage())
	}
...
func Delete[model any](tr *Transaction, opts ...Option) (int64, error) {
	...
	resp, err := tr.pg().Repo(context.Background(), req)
	if err != nil {
		return 0, fmt.Errorf("gistpostgres: delete: %w", err)
	}
	tr.captureHandle(resp)
	if resp.GetErrorCode() != "" {
		return 0, fmt.Errorf("gistpostgres: delete: %s: %s", resp.GetErrorCode(), resp.GetErrorMessage())
	}
	return resp.GetAffected(), nil
}
...
func DeleteWithReturning[model any](tr *Transaction, opts ...Option) ([]model, int64, error) {
	...
	resp, err := tr.pg().Repo(context.Background(), req)
	if err != nil {
		return nil, 0, fmt.Errorf("gistpostgres: delete with returning: %w", err)
	}
	tr.captureHandle(resp)
	if resp.GetErrorCode() != "" {
		return nil, 0, fmt.Errorf("gistpostgres: delete with returning: %s: %s", resp.GetErrorCode(), resp.GetErrorMessage())
	}
	models, err := decodeRows[model](resp.GetRowsJson())
	return models, resp.GetAffected(), err
}
...
func decodeRows[model any](rowsJSON [][]byte) ([]model, error) {
	...
	for i, rowJSON := range rowsJSON {
		if err := json.Unmarshal(rowJSON, &models[i]); err != nil {
			return nil, fmt.Errorf("gistpostgres: could not decode row: %w", err)
		}
	}
	return models, nil
}
